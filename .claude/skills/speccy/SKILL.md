...
- While fixing, re-run **only the specific test(s) you just touched or that were failing** — never the whole suite. Re-running every test to confirm a one-line fix in one test is waste; it does not happen.
...
- Concretely: fix test/class X → run X (and its production class's targeted tests) → green → move on. Only when the whole fix list is exhausted do you run the entire suite once to confirm nothing regressed.
- For *intermediate* checks inside a fix loop prefer the cheapest signal that still proves the fix — compile / type-check / `--dry-run` / the single changed test. Batch several fix findings into one fix agent and re-check once, rather than re-gating after each small edit.

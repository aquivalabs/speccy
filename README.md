...
| **2. Plan** | Autonomous | speccy re-reads the spec's external references — the tracking issue and its comments, the docs it cites — and puts anything that has moved past the spec to you before planning starts. Then a subagent researches the codebase and drafts a plan (the *how* to the spec's *what*), running feasibility spikes to prove any risky mechanism before planning around it. |

...
| **2. Plan** | Autonomous | A subagent researches the codebase and drafts a plan (the *how* to the spec's *what*). |
...
- **Adversarial critique at every artefact boundary.** Specs, plans, and the final diff each get an independent reviewer whose job is narrow: read one artefact, find specific problems. Each loop runs up to 4 rounds and **exits early** when a round surfaces nothing valuable, so it runs only as long as each round keeps earning its place.

- **A per-phase model "ladder" that ends on doubled Opus.** Spec and plan critique use `sonnet → sonnet → opus → opus` (skipping the cheapest tier — a cheap model's false findings cost more triage than they save on knowledge-heavy domains like Salesforce). Implementation review uses `haiku → sonnet → opus → opus` since the diff is large and many findings are mechanical. The doubled Opus is deliberate: round 4 is a *fresh-context* Opus re-reviewing the artefact after round 3's fixes — a genuine independent pass that reliably catches what the first Opus missed. The **builder** defaults to Sonnet; the plan-execution breakdown agent always uses Opus because decomposition is the hardest-thinking step. All overridable per run.
...
Speccy is built for the multi-hour build that would blow out a single context window. The resumability and thin-orchestrator design keep the main context small enough to go the distance, and the per-phase model ladder keeps the bill down by spending Opus only where it earns its keep.

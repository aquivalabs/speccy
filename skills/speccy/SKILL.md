...
1. **Walkthrough or start?** Tell the user they can ask for a walkthrough of the process, or just describe what they want to build to get going. If they ask for the walkthrough, explain each phase in a few sentences, organised around what the user does vs what runs autonomously:
   ...
   - **Implementation** (autonomous loop): the skill builds to the plan; parallel reviewers check the code across several lenses (correctness, spec fidelity, tests, codebase fit, local-doc adherence, comment noise, strict scrutiny of linter/analysis suppressions, and the repo's own review gate when it ships one); fixes are applied directly, or deferred as future work.
...
- **Implementation review**: parallel review lenses, up to 3 rounds (see Phase 4). The five judgment lenses (correctness, spec fidelity, tests, codebase fit, local-doc adherence) run on opus, and the suppressions and comments lenses on sonnet. Any inline gate the run adds — the repo's own review gate, or the built-in `code-review` skill — manages its own models.
...
After implementation is complete, the code gets an independent review across several lenses, run in parallel. Completeness is already verified by the task execution skill, so this phase is about correctness, quality, spec fidelity, and fit.
...
Pass each bespoke lens `prompts/review-output-contract.md` alongside its own prompt. It standardises the finding shape across lenses so triage is mechanical, and makes writing the file a hard contract: a lens that runs out of room mid-verification still leaves a file, marking the unconfirmed candidate `PLAUSIBLE`, rather than returning nothing. An inline gate is another skill and won't read the contract; the orchestrator applies the same shape itself when it normalises that gate's findings into its lens file.

- **Correctness**: `prompts/review-correctness.md`. Bugs, logic errors, inputs and boundaries, error handling, silent failures, concurrency and lifecycle, resource leaks, and behaviour the diff removed. The panel's floor: whether the code works, judged apart from whether it is tidy, tested, or faithful to the spec.
...
- **Local-doc adherence**: `prompts/review-local-docs.md`. Violations of the repo's governing docs, including CLAUDE.md, which it re-reads deliberately even where another lens has touched the same rule.
...
- **Project review gate**: the repo's *own* review gate, if it ships one (a `/review`-style skill, project-defined reviewer agents, or a `.claude/review.config.json`). When present, run it as an extra lens. It encodes the house security bar, thresholds, and invariants a generic reviewer can't replicate, so where it exists it is the highest-signal lens in the panel; run it the way the repo documents (its own agents, models, and thresholds; do not override them). A project gate is usually itself multi-agent, so invoke it **directly in the main conversation** rather than wrapped in an Agent subagent: wrapping a skill that spawns its own children stalls it (see **Subagent results: trust files over returns**). It is **spec-blind**: it checks house quality and says nothing about whether the build meets _this spec's_ criteria, so it complements the spec-fidelity lens and never replaces it. Normalise its findings into the shared shape and write `review-round-N-project-gate.md` yourself; the triage step dedups its overlap with correctness, codebase fit, and local-doc adherence like any other lens. If the repo has no such gate, skip this lens.

Run the bespoke lenses on **opus**, except suppressions and comments on **sonnet** (a mechanical scan, and a focused style pass). A pinned adversary model overrides all of them; an inline gate manages its own.

### The optional `code-review` gate

The built-in `code-review` skill is not part of the panel. It used to own correctness here, and it stopped reporting back: on a three-round review it returned only its intermediate message every round ("the finders are running"), the dedup and verify stage never arrived, and its finder angles landed as loose task notifications. The correctness lens above owns that ground now, so no round waits on it.

Add it when the user asks for a second correctness pass, or when the diff is large enough to be worth one. Then:

1. Invoke it **directly in the main conversation** (via the `Skill` tool), targeting `<base-branch>...HEAD` at `high` effort, with no `--fix` and no `--comment`. Never wrap it in an Agent subagent, for the reason the project gate above gives.
2. If its consolidated result arrives, parse it tolerantly (the shape may change), normalise its verdicts into the shared finding shape, and write `review-round-N-code-review.md` yourself.
3. **If it doesn't, the panel's own completion is the deadline.** Once every spawned lens file is present, stop waiting and write the file from what the finder angles reported: their notifications, and anything they left on disk. Mark every finding `PLAUSIBLE`, whatever confidence a finder claimed. Finder output has not been through the verify stage that turns a candidate into a finding, so it is a lead rather than evidence. Tell any fixer that takes one to confirm the defect in the code first, and to report back rather than change anything if it doesn't hold. If nothing arrived at all, write the file saying so. Never hold a round open for this gate.

### The loop (up to 3 rounds)

1. **Review.** Spawn the bespoke lenses as parallel subagents in one message, and invoke in the main conversation whichever inline gates this run uses (see above): the project review gate if the repo ships one, and `code-review` if the run added it. Their own fan-out overlaps with the spawned lenses. **Every round is a cold review**, rounds 2 and 3 included. Point each lens at the whole diff again, and add the round-(N-1) fixes as further ground to cover: verify they hold, and catch any regression they introduced. A round that *only* verifies the last round's fixes can never find what the panel missed the first time, and one cold pass is not enough: findings well within the panel's reach routinely surface in a later independent review of the same code. Always pass the `.speccy/<run-id>/deferred.md` list as accepted decisions it must not re-raise.

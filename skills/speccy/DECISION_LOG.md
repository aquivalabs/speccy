...
### The panel owns correctness; `code-review` becomes an optional extra (2026-08-26)

Reverses the delegation in "Implementation review becomes a parallel multi-lens panel, delegating code quality to the built-in `code-review` skill" (2026-07-13). The lens taxonomy that decision derived still stands; the dependency at its centre does not.

`code-review`'s consolidated verdict stopped arriving. Across a three-round review it returned only its intermediate message every round — the finders are running — and the dedup and verify stage never reported; its eight finder angles landed as loose task notifications instead. The orchestrator normalised raw finder output into the lens file by hand, marked all of it `PLAUSIBLE`, and had each fixer confirm a finding before acting on it. That recovery worked, but the skill anticipated none of it: it said "write the file yourself" on the assumption that a forked skill's final text arrives.

Running the skill inline (2026-07-14) had already removed the wrapper as the fragile layer. What was left is the dependency itself, and no contract speccy can write reaches inside another skill's fan-out — the write guarantee in `review-output-contract.md` binds the agents speccy spawns and nothing else. So `prompts/review-correctness.md` takes the ground back: bugs, logic errors, inputs and boundaries, error handling, silent failures, concurrency and lifecycle, resource leaks, and behaviour the diff removed. It is a bespoke lens like the other six — one prompt, one file, the shared write guarantee, opus, spawned in the same parallel batch — so the panel's floor no longer depends on anything returning.

`code-review` stays available rather than being deleted, as an **optional** extra pass: its finder angles are good, it is maintained outside this repo, and a second opinion on correctness over a large diff is cheap. Two things change. It is additive, so nothing in the loop is scheduled around it. And its failure is written down instead of improvised: a wait bounded by the panel's own files, then collect what the finders reported, normalise it into the shared shape marked `PLAUSIBLE` whatever confidence a finder claimed, and tell the fixer to confirm the defect in the code before changing anything. Unverified finder output is a lead, not evidence.

The accepted cost is a prompt speccy now maintains on the axis that moves fastest, against a skill that was improving for free. A panel that returns a verdict is worth more than one whose best lens might not.

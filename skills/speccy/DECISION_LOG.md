...
### Metrics are read from the transcripts, not recorded by the run (2026-08-18)
...
The obvious shape, a `tokensUsed` field written at each phase boundary, is impossible rather than merely unwelcome. Nothing in a live session exposes token counts to the orchestrator: the Agent tool result carries `{agentId, resolvedModel, outputFile, status}` and no counts, and the main loop has no view of its own usage. Any figure speccy wrote as it went would be invented. That settles the question the closed schema would otherwise have to argue: measurement happens after the run, from the harness transcripts, in a file of its own.

Phase attribution needs no new state either. The transcript already records every state.json write with its content and a timestamp, so the phase timeline is derivable from what the run does anyway. `metrics.md` is written once at wrap-up for the user to read at handoff. No critic reads it, no resumed context reads it, nothing branches on it, which is what keeps it out of the inert-content trap that closed the schema (see **state.json points at the run, it doesn't record it**).

Node, not Python or awk. A dependency-free reader was the appealing option and it is unsafe: `usage` embeds an `iterations` array repeating the same counts, so a text-regex pass over a real corpus reported 5,418,443 output tokens against a true 2,709,266. Exactly double, with no error raised, which is the worst failure a measurement tool can have. Real JSON parsing is required, and only the top-level `usage` fields may be read. Between the two runtimes that parse JSON with no install, Node is the better bet for this audience and matches the ecosystem the plugin ships in; Python's name in Git Bash on Windows is a coin toss between `python3`, `python`, and `py`. Neither is guaranteed, so the wrapper skips with one line rather than failing, exactly as the banner does.

No prices. Rates in a public plugin go stale silently, and the report groups tokens by model and reasoning effort, which is what applying current rates by hand needs. `effort` turned out to be recorded per request, absent only for models with no reasoning-effort setting, so it is grouped alongside the model rather than assumed.

Two things real runs taught the reader that a synthetic fixture never would. Subagents carry an `agent-<id>.meta.json` sidecar naming what they were spawned to do, but workflow subagents get no description, so 26 of one run's 91 agents showed as hex ids; the label now falls back to the agent's opening prompt. And a run whose earliest surviving state write set `review` had its whole first half labelled `spec`, so a timeline starting mid-pipeline now names its opening bucket `before <phase>` and says in the report that those phases cannot be told apart.

### Metrics run after `complete`, and scope their agents to the run (2026-08-19)

Metrics were an exit check, so `complete` could not be set until the report existed. That put a nice-to-have between the user and a finished run, and it also made the report worse. With the run still short of `complete`, every reading closed its last phase at "the last thing the session did" and carried a note warning that the wall figure might span days of unrelated work. The ordering guaranteed that note on every run, so the one anomaly it exists to flag had become the normal case.

Running the script after `complete`, outside the exit checks, fixes both. The measurement can fail, skip, or be interrupted without leaving a run that looks unfinished, and the `complete` boundary is there to close the timeline. The cost is that a `/clear` in the gap loses the report, which is why the script takes a run id: the transcripts still hold it.

The same reading settled which subagents belong to a run. A session outlives the run it hosted and keeps every subagent it ever spawned in one directory, so a session that ran speccy and then did something else was lending the report agents that had nothing to do with it. Their tokens were already excluded from the phase totals, which left the agent table disagreeing with the figures above it, and since the table sorts by output the largest stray sorted to the top. Agents are now scoped by overlap with the run window, and the excluded ones are counted in the notes rather than disappearing. Work done after the run is real, but it is not this run's cost, and a report that mixes the two can answer neither question.

### state.json says whether a run finished; the transcript says when each phase began (2026-08-19)

The first real run measured reported itself unfinished while its own state.json said `complete`. The wrap-up had set that phase with a `python3 -c` one-liner rather than the Write tool, and the timeline is read from tool calls, so the closing moment left no trace. The last phase then ran on to the last record in the session and swallowed ten records of work done after the run was over.

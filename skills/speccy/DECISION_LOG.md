...
### Every Bash call is one plain command (2026-08-26)

A sandboxed or worktree-isolated session refuses any Bash it cannot verify stays inside the sandbox, and what it refuses is not exotic: `&&` chains, `for` loops, heredocs, `cmd; echo $?`, and the `source ~/.nvm/nvm.sh && nvm use 24 && …` prefix a project's own `CLAUDE.md` may prescribe. Every fix agent in one three-round review lost several round trips rediscovering that, each refusal costing a turn and teaching nothing that survived into the next agent's context. plan-execution's watchdog fared worse: its `Monitor` wanted a poll loop, which is a compound command by construction, and it was refused twice and never armed.

The rule went into `tool-use.md` rather than into each prompt. That is the same call the parallel-gathering rule made, for the same reasons: the evidence is uniform across build agents, fixers, critics and lenses, so there is no divergence to encode, and the file is already passed to every subagent under a rule stated once. It is also the one prompt whose scope reaches plan-execution's build agents, which is where most of the refusals landed.

Two existing rules become instances of it rather than advice of their own. The banner and the metrics script were each told not to prepend `cd` and not to use command substitution, for a different reason: a pre-approved permission entry matches the literal command, so either turns an allowed call into a prompt. Two reasons, one shape. Both spots now name the general rule instead of restating half of it, which is what stops the next such rule from being written a third time.

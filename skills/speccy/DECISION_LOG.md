...
Two design choices shaped by the tooling. The `readabilityPasses` field changed from `["spec"]` to `[{ artifact, atRound }]`: the invariant "a critique round ran after the readability pass" was asserted four times in prose with no field to back it, and recording the round the pass ran at is what lets `advance` check `counter > atRound`. And `state.mjs` writes the file itself via node, which the transcript records as a `Bash` call with no `file_path` — invisible to `metrics.mjs`, which detected boundaries and included sessions by matching a `Write` of `state.json`. Rather than have the agent write the JSON (which would keep it hand-editing state), `metrics.mjs` learned to read the CLI's command: the runId rides every call as `--run <id>`, so boundaries stay scoped and timestamped from the transcript exactly as before, and the existing Write/Edit path stays as the fallback for a hand-written state. The phase enum now lives once, in `state.mjs`, which `metrics.mjs` imports.

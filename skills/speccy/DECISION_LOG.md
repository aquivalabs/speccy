...
### The run records which checkout it is running in (2026-08-26)

Nothing in state.json said where a run lived, so every git-mutating step resolved against the shell instead. Sequential plan-execution tasks run on "the main checkout", which in practice means the session's working directory; fix batches and the orchestrator's own `git commit -am` mean the same thing. A run whose branch sits in a git worktree, the ordinary arrangement when the main checkout is shared with other sessions, has every one of them pointed at the wrong branch.

One run showed both halves. The build was about to start against a main checkout another session had switched to the default branch with uncommitted edits, caught only because someone checked by hand before invoking plan-execution; `EnterWorktree` fixed it, and nothing in the skill had suggested it. Earlier in the same run a `git commit -am`, after the shell's working directory had reset, swept an unrelated modified file into a commit on the default branch. `.speccy/` was resolvable from two directories at once, the same files by coincidence rather than by design.

So state.json gains `checkoutPath` and `workBranch`, and the orchestrator confirms `git rev-parse --show-toplevel` and `git branch --show-current` against them before every commit, every fix batch, the plan-execution invocation, and every merge. A mismatch enters the recorded checkout or stops. It never proceeds, and it never resolves itself by rewriting state to match the shell, which would turn the record into an echo of the thing it exists to check. Subagent prompts pass `git -C <checkoutPath>`, under the same stated-once rule that carries `writing-style.md` and `tool-use.md` to every spawn site.

**The closed schema is the interesting part.** Its rule is that a field belongs in an artifact, and the routing exists because content parked in state is inert: no critic reads it, the user never sees it, the wrap-up distils the artifacts. These fields fail that test in the other direction. An artifact records what the run decided, and where the run is running is not a decision; no spec section, plan section, or decision-log entry could hold it. It is also the one fact a resumed context cannot recover for itself, because the shell it wakes up in is exactly what may be wrong. So the schema stays closed, with a stated reason why locating the run is not a precedent for recording anything else about it.

`transcriptDirs` is the third field, and it exists because the same move breaks the metrics reader. The harness files a session's transcript under a directory named after its working directory, so a session that moves checkout mid-run splits its transcripts across two of them; that run's metrics reported no transcripts found for work that had plainly happened. The reader now starts from the recorded directories, falls back to deriving one from the working directory, and sweeps every project directory when neither turns anything up, saying in its notes that it had to. A list rather than a single value follows from what goes wrong: a run that moves has transcripts in both places, so the field appends where `checkoutPath` is rewritten.

Nothing was added to `allowed-tools`. `git rev-parse` and `git branch --show-current` sit where `git commit` and `ls` already sit, approved by the auto-accept mode the skill asks for before the autonomous phases begin.

Two things this deliberately does not do. speccy does not create the worktree: which checkout a run belongs in is the user's arrangement, and the skill's job is to notice it and hold on to it. And the metrics reader does not search for the checkout, which would mean guessing a repository root from a working directory that is already suspect; the guard is what makes the working directory trustworthy by the time the reader runs.

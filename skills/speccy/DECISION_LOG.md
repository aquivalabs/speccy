...
### The user's first read of the spec is a stop (2026-09-09)

The run kept spawning the spec critic before the user had reviewed the first draft. The user's first read was the one interactive moment with no stop behind it: everywhere else a pause is enforced by an engagement question, hardened by "ask it as the last thing in the turn and wait", and 1c deliberately removes that question because it isn't a gate. So "let the user read and edit until satisfied" sat as a mild aside with nothing to hold the flow, and the next instruction, 1d step 1, spawns the adversary. The pre-question meant to create the pause is in 1d step 2, after the spawn, and asks the user to predict the critique, which presumes the critique already exists; it can't stand in for a first review. The `phase` enum has no value for the first read either, so neither the live flow nor a resumed run had anywhere to hold.

The fix keeps 1c a non-gate (no engagement question) but ends it on an explicit stop, borrowing the gate stops' own language: put the hand-off prompt last and wait until the user hands the draft on. The first read also gets its own phase, `spec-draft`, so the stop is backed by the state model rather than the instruction alone. The run's bookkeeping (branch, committed draft, runId, state.json) now happens before the stop, so state.json is written as `spec-draft` when the run is created and flips to `spec-critique` only when the user hands the draft on; a `/clear` mid-read resumes at `spec-draft`, which re-presents the draft and stops again. Leaving the stop in the instruction with no phase was the smaller change, but a run is resumable only at a recorded phase, so without `spec-draft` a mid-read `/clear` would resume at `spec-critique` with the critic already implied, which is the jump this fixes.

The first recordable phase is named a second time in `metrics.mjs` (`FIRST_PHASE`), which uses it to tell a normal run's opening bucket from a timeline that lost its early state writes. It moves to `spec-draft` with the schema. The metrics now bucket the first read as its own phase, distinct from the `spec` bucket that holds the intake, interview, and drafting before any state write.

### The run state is owned by a CLI, not hand-written prose (2026-09-09)

The orchestrator hand-assembled `state.json`: it wrote the field list at 1c, flipped `phase` at each boundary, bumped counters, and ran prose "Exit checks" that were trust-the-agent. The `spec-draft` bug above was one symptom (a phase the model was told to write but the enum had never named); the deeper cost was that every guard was advisory and the phase set lived in two files. `state.mjs` takes the writes: `init`, `advance`, `record-round`, `record-readability`, `engagement-add`, `set-model`, `replan`. The agent still reads `state.json` directly (a plain Read of JSON needs no verb) and keeps every judgment; the CLI owns only the deterministic contract.

**It enforces invariants and records policy.** `advance` refuses a transition whose mechanical guards fail (a spec uncommitted, no critique round after the readability pass), so the exit checks are executable rather than a list the model can skip. But it never caps rounds or forbids an extra readability pass or a re-run, because those are the agent's divergences to make; a guard that could block a legitimate state is policy mis-filed as an invariant. That line is why the file-existence checks for `spec-critique-skipped.md` and `deferred.md` are *not* guards: a clean run with nothing skipped legitimately has no such file. And the plan, which lives in gitignored `.speccy/`, is guarded for existence rather than for a commit.

Two design choices shaped by the tooling. The `readabilityPasses` field changed from `["spec"]` to `[{ artifact, atRound }]`: the invariant "a critique round ran after the readability pass" was asserted four times in prose with no field to back it, and recording the round the pass ran at is what lets `advance` check `counter > atRound`. And `state.mjs` writes the file itself via node, which the transcript records as a `Bash` call with no `file_path` — invisible to `metrics.mjs`, which detected boundaries and included sessions by matching a `Write` of `state.json`. Rather than have the agent write the JSON (which would keep it hand-editing state), `metrics.mjs` learned to read the CLI: it scopes a call to the run by the `--run <id>` on the command, then reads the landing phase from the call's *result*, the line `state.mjs` prints on success (`advanced Y -> X`, `initialised … at phase X`, `replanned: phase -> X`). Reading the result, not the command, is what makes the timeline honest about what happened rather than what was attempted: a refused verb writes only to stderr and exits non-zero, so it leaves no such line and records no boundary, and a `replan` back to `planning` is a boundary the command's verb alone would have missed. The success words (initialised, advanced, replanned) never appear in a refusal. The existing Write/Edit path stays as the fallback for a hand-written state, and the phase enum now lives once, in `state.mjs`, which `metrics.mjs` imports.

The CLI fails loud (non-zero exit, a `speccy state:` line), the opposite of `metrics.mjs`, which never blocks a run: state is load-bearing, so a refused transition must stop the agent rather than pass silently.

### The phase bodies load on demand, per phase (2026-09-09)

`SKILL.md` was read whole on every trigger, but a run `/clear`s at phase boundaries and resumes with only the recorded phase to run. The spec phase never needs the review lenses; the build phase never needs the interview. So most of the monolith was dead weight in any given window, and the heaviest interactive stretch (spec) paid to carry the build's detail it would not reach.

The split puts each phase's body in its own file: `phases/spec.md`, `phases/plan.md`, `phases/build.md`. `SKILL.md` keeps the core that every window needs regardless of phase (getting started, resuming, preconditions, the state-CLI contract) plus a dispatch table mapping the `phase` in `state.json` to its file. A resumed context loads that core and reads one phase file, roughly half the old skill per window when a run clears at each boundary.

The cross-cutting rules stay in the always-loaded core, not in the phase files, and that placement is the load-bearing choice. Every phase and every subagent spawn leans on them (writing style, the decision-log discipline, trusting files over agent returns, propagating the session's voice, the tool-use pass, and the engagement habits under "Steering away from cognitive surrender"); a phase file that restated them would drift copy by copy, and one that omitted them would run a phase without them. So the phase files layer on top of the core and reference it by section name. Two costs are accepted for the saving: the dispatch adds one Read per phase, which is the mechanism rather than waste; and the sum across all four files runs about 3% over the old monolith (the dispatch table, the per-file anchors, the repeated headings), so a run that never clears pays a little where a clearing run banks the cut. `build.md` is the largest, carrying Phases 3, 4, and wrap-up together; if that phase comes to dominate real runs, splitting the review panel out is the next move.

### The revise agent runs on sonnet, not opus (2026-09-22)

Supersedes the revise-agent portion of "Spec and plan critique run all-opus, three-round cap" (2026-06-22). The spec and plan revise agents now run on sonnet; the adversary stays on opus every round.

Applying a set of already-triaged findings against the style guide is execution against a written instruction, the same class of work as the readability pass that already runs on sonnet. The hard thinking is choosing which findings to accept and why, and that happens in the main thread before the agent is spawned; the agent edits, it does not judge. The 2026-06-22 argument for opus-revise, that it "keeps the artifact's edits at the same standard as the critique", conflated producing findings with applying them; only producing them needs the top tier.

### The plan's user review is its own phase (2026-09-22)

A run that converged the plan critique and `/clear`ed before the user approved resumed thinking the loop was still open, and re-ran a critique round against a settled plan. The cause: `plan-critique` named two situations the resume could not tell apart. The autonomous critique loop (2a) and the user-review gate (2b) both ran under it, and the advance to `implementation` fired only at the end of 2b. Convergence itself is a judgment the live agent makes by reading the round's critique file, and nothing persisted it, so on resume `plan-critique` with `planCritiqueRounds: 2` described equally "round 2 found nothing, at the gate" and "round 2 had findings, round 3 due". A cold context reading "up to 3 rounds" with the count at 2 runs round 3, and because 2a is autonomous, the spurious round mutates the plan before the user sees it.

The fix makes the gate a real boundary: `plan-review` sits between `plan-critique` and `implementation`. The loop lives in `plan-critique`; on convergence the run advances to `plan-review` and the user review runs there; approval advances to `implementation`. Resume is now unambiguous from the phase alone: `plan-critique` is still looping, `plan-review` is converged and at the gate. The loop-exit invariant (plan present, a critique round after the plan readability pass) moves onto the `plan-critique -> plan-review` edge, where it belongs; `plan-review -> implementation` re-checks only that the plan still exists. `replan` gains `plan-review` as a legal origin, since a concern raised at the gate is a re-plan.

The spec loop has the same shape (`spec-critique` covers the loop and the post-loop exit) but a narrower window: every spec round already stops for the user, and convergence there is adjacent to the advance to `planning` with no autonomous sub-phase between. `phases/spec.md` already advances to `planning` on exit before offering the `/clear`, which closes almost all of it, so the spec side is left as prose rather than given a second gate phase; a converging plan is the case that ran unattended and did damage. Splitting the phase rather than persisting a "converged" flag was chosen because the resume decision then lives in the state model, not in prose the agent has to remember to consult, which is the layer that failed here.

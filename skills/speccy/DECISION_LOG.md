...
### A fix round is checked for its own residue, and the last round's fixes ship unreviewed (2026-08-26)

Three cold rounds exist to find what one round misses. What they mostly found was each other. Across one three-round run, most of round 2's findings were residue of round 1's fixes and most of round 3's were residue of round 2's, rather than defects in the build: six sites in a single round cited review finding ids in comments, pointing at a gitignored directory nobody reading the merged branch can open; a "promote to shared" fix wrote the shared copy and left the original and its importers standing; an export whose only consumer had been deleted survived twice, in different modules, still carrying a comment describing the mechanism that had gone; and a fix landed on the loading branch and not on the error branch one line below it.

Each of those is cheaper to grep for than to review for, which is what decides the shape. The four failures become rules in the fixer's prompt, along with a bounded re-read that catches what a fix made redundant. The orchestrator then runs three greps over the run's own diff once the round's batches have committed: review ids in the source, exports whose only importers are tests, and leftover conflict or `TODO` markers. Hits route straight to a small fixer batch. They do not wait for the next round's triage, where a whole lens pass would be spent rediscovering what a grep has already named, and where at the last round there is no pass left to spend.

**That last point forced the contract the skill and the orchestrator had been reading differently.** The skill implied a review-only final round, its Fix dispositions all landing in `## Unaddressed at the round cap`; an orchestrator with findings in hand naturally fixed them instead. Review-only is honest, but it ships defects one batch would have closed and it turns the cap section from "the rounds ran out" into "nobody applied these". So the final round dispositions like any other and gets a bounded fixer series, the gates, and the residue check. No fourth cold round opens over those fixes, which leaves them gated but never independently reviewed — the wrap-up now says so rather than leaving the user to work it out. A fourth round was the alternative and it only moves the boundary: whatever it fixed would be unreviewed in exactly the same way, one round later.

### The panel is given the decisions it may not reopen (2026-08-26)

`deferred.md` tells a lens which findings not to re-raise. Nothing told it which decisions were closed, and a decision is what the lenses kept reopening. Over three rounds: a first-paint gate the plan argued for, an allowlist the plan chose over lazy loading, a wire mapping it chose over serialising a class, and a guard a previous round had deliberately added — each returning from a different angle, often through a different lens. Every return cost a triage line and a declined entry, and the declined entries then had to be re-fed to the next round by hand.

`settled.md` is the missing input. It is written before round 1 from the plan's Architecture decisions, which have already survived the plan critique and the user's approval, and each triage appends what it declined. Declining becomes a disposition of its own rather than a quiet drop, which is what keeps the file honest: a finding that is simply wrong stays a false positive, since settling a mistake would bind the panel to it.

Pooling it into `deferred.md` was rejected for the reason the spec-critique skips were kept out of that file (2026-08-14): one file, two meanings, one reader. Deferred says a real finding belongs to another PR. Settled says an argument was heard and closed.

The rule the lenses get is the load-bearing half, because a list that closed off evidence would cost more than the churn it replaces: arguing against an entry is out of scope, and showing an entry is *broken* is in scope and must name the entry. It lives in the prompts rather than in the orchestrator's spawn preamble, since `deferred.md` was passed the second way and the lenses re-raised it anyway; triage still drops re-raises as the backstop. Two lenses bind differently, and deliberately. The suppressions lens is bound only on the entry's own terms, so a suppression that has since widened is in scope again. The local-docs lens outranks the list outright, because a settled decision that violates a governing doc is exactly what that lens is for.

...
Applying a set of already-triaged findings against the style guide is execution against a written instruction, the same class of work as the readability pass that already runs on sonnet. The hard thinking is choosing which findings to accept and why, and that happens in the main thread before the agent is spawned; the agent edits, it does not judge. The next critique round then reads the result cold, so a dropped or garbled edit is caught (for the plan's final-round revise, where no further critique follows, the 2b user review is the backstop). The 2026-06-22 argument for opus-revise, that it "keeps the artifact's edits at the same standard as the critique", conflated producing findings with applying them; only producing them needs the top tier.

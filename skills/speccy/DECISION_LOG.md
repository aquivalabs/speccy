...
### Planning re-reads the spec's references before it plans (2026-08-26)

A run planned against a surface that had been wrong for days. The open question about which surface the feature attached to had been answered by a stakeholder in a comment on the tracking issue, somewhere between intake and planning. The spec still carried the intake answer, nothing in the pipeline read the issue again, and the planner did exactly what it should: it took the spec at its word. The plan critique then hardened a plan for the wrong surface, and the correction arrived from the user at the plan-review gate.

The gap is structural rather than that run's oversight. Intake is the only step that reads external context; everything after it reads the spec. Between the two sit the interview, up to three critique rounds, and the context clear the skill itself recommends. On a live feature that is long enough for the answer to a question the spec is built on to land somewhere the spec cannot see.

So Phase 2 now opens by re-reading what the spec names and comparing it against the spec's own premises — the surface, the scope boundaries, the assumptions — and a difference is a blocking question before the planner spawns. It is the same shape as the contradicted-assumption gate (2026-06-26), for the same reason: a premise that moved can invalidate scope, and the cheapest moment to learn that is before a plan exists. The user's choices are the same two, and correcting the spec runs the same revise step the spike delta uses.

Two things keep it cheap. It re-reads a **recorded list** rather than hunting for context: each reference once, no following links onward, nothing sought that the spec doesn't name. That is also why 1b now records the tracking issue alongside the docs and standards it already collected — a refresh can only re-read what intake wrote down. And an unreachable reference is not a difference. It is named to the user, recorded, and passed to the planner for the plan's Risks, because a session that lacks access to one doc would otherwise be unable to plan at all.

Refreshing at every phase boundary was the obvious generalisation and it is worse. The phases after planning read the plan and the code, not the spec's premises, so there is nothing there for a difference to change cheaply, and a gate that fires four times is one the user learns to wave through. Planning is the single boundary where a moved premise is both still cheap to act on and attached to a decision.

### A spike that moves the spec revises it and re-plans the delta (2026-08-26)

The same run, one phase earlier. The planner's first spike confirmed a mechanism the spec expected it to refute: the spec had written a workaround into its deliverables to accommodate a limitation that turned out not to exist. That is the false-constraint case "False constraints are proven before the design works around them" (2026-07-09) exists to catch, caught where it intended. What followed was the problem. The skill's only route from there was "revise the spec and re-plan": the spec was rewritten by hand, planning restarted from zero, and three fresh critique rounds hardened a plan that was substantially the one already hardened.

Two separable things were missing.

**The spec revision had no mechanism.** `revise.md` already applies accepted findings to an artifact, and a spike verdict is a critique of the spec in everything but name, so the spike file is now handed to it as the critique file. The scope ruling stays with the user at the blocking gate that already fires; the corrections the verdict forces outright — an assumption now false, a constraint the environment refuses — are mechanical and apply without one. `revise.md` needed one paragraph for that, because "ignore the rest of the critique: those were considered and rejected" reads wrong over a file that is evidence rather than opinion, and would license dropping a fact the user never rejected.

**Re-planning had only one gear.** Delta mode is the second: the planner is handed the superseded plan and the verdict, and changes what the verdict changes. Decisions it leaves standing carry forward in their existing wording, with their rationale and origin tags, because those survived critique rounds that a rewrite discards silently and re-earns at full price. The appendix carries too, minus what the new design invalidates. The output is still a complete plan rather than a patch: it is read by a human at 2b and by breakdown agents afterwards, and a plan that reads as a diff leaves two documents to reconcile and gives the next critique round the wrong thing to review.

The trigger is what a verdict moves, not which of the three words it carries. `confirmed` moved the spec in the run above, so a verdict word cannot be the gate. `plan-spike.md` therefore ends by naming the spec assumptions the verdict contradicts and the deliverables it reshapes, or saying it reaches neither, and the orchestrator routes on that line from either spike site — the planner's own or 2a's.

The bookkeeping reuses what exists. The superseded plan and anything written against it take the `SUPERSEDED-` prefix already defined for a replaced artifact, renamed before the planner runs so the new plan takes `plan.md` and `planPath` never moves; the spike file keeps its name, being evidence about the world. `planCritiqueRounds` resets to 0, since a delta plan inheriting the round count would inherit a clean sheet the old plan earned. `readabilityPasses` does not reset: the pass rewrites prose, the delta plan carries the passed prose forward, and the critique rounds that follow read the new sections cold, which is the check the pass is owed.

A full re-plan stays available for the user asking and for a changed surface. Where the verdict rewrote the deliverables there is little left to carry, and a delta framing would anchor the planner to a shape that no longer exists.

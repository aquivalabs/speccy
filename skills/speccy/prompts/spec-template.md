...
<!-- Follow `writing-style.md`. The sections run description first, argument second, caveats last —
     keep them that way: rationale belongs in Decisions & rationale, not spread through the rest.
     Don't restate `CLAUDE.md` anywhere in this file. Reference it by file/section where a constraint
     turns on it, and spell a rule out only where this feature diverges from it. -->
...
<!-- This is the file's lead. One paragraph: the problem this solves and what the spec produces.
     ...
     a hard limit, not a target: a third belongs in Decisions & rationale like the rest.
...
<!-- Observable conditions that mean the work is done: verifiable by running something, reading
     something, or checking something concrete. Not a restatement of deliverables — these are the
     specific checks.
...
<!-- The load-bearing choices that shaped this spec, and why each beat the alternative. This is the
     ...
     - **Origin** — **User** (a preference, mandate, or judgement the user brought) or **Speccy,
       ...
       *Speccy, alone*, arises in the plan and review phases, not here. The wrap-up reads this tag.
     ...
     Keep it spec-level: WHAT to build and the shape of it, not code-level HOW (that is the plan's).
     ...
     State the decision as it now stands. A choice this spec reached and then reversed is history for
     the decision log, not content for this section. -->

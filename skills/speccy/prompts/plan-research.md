...
- **Settle what the repo's own tooling does by running it.** A field's documented meaning is one question; what *this* project's script, hook, or gate actually does — which file it reads, what makes it exit non-zero, what it returns — is another, and only the second one breaks a build. Reading a name, a docstring, or one function of a longer script confirms a plausible story instead of the real one. So for every behaviour the plan will lean on, run the thing (or the smallest command that exercises the behaviour) and plan from the actual output.
